...
func cast_spell(owner: SpellManager, index: int) -> void:
	...
	if is_instance_valid(owner.source):
		var target: Vector3 = owner.source.global_position + owner.source.skin.global_basis.z * 30.0
		var shape_cast: ShapeCast3D = ShapeCast3D.new()
		owner.source.skin.add_child(shape_cast)
		...
		if shape_cast.is_colliding():
			owner.source.global_position = shape_cast.get_collision_point(0) - owner.source.skin.global_basis.z
		else:
			owner.source.global_position = target
		shape_cast.queue_free()
	else:
		print("no owner target on blink")
