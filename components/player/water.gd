...
func state_physics_process(delta: float) -> void:
	...
	var swim_up: bool = Input.is_action_pressed(&"jump")
	var swim_down: bool = Input.is_action_pressed(&"crouch")
	...
	if parent.velocity.y > 0.0 and water_ray.is_colliding() and water_ray.get_collider() is WaterArea:
		var distance: float = water_ray.get_collision_point().distance_to(water_ray.global_position)
		if distance > 0.1:
