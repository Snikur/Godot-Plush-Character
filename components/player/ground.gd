...
func state_physics_process(delta: float) -> void:
	...
	var vel_2d: Vector2 = Vector2(parent.velocity.x, parent.velocity.z)
	if movement_input != Vector2.ZERO:
		if parent.is_on_floor():
			parent.transition_to(parent.AnimationState.RUN if is_running else parent.AnimationState.WALK)
	...
	if parent.is_on_floor() or coyote_timer.time_left > 0.0:
		if Input.is_action_just_pressed(&"jump"):
			coyote_timer.stop()
			parent.transition_to(parent.AnimationState.JUMP)
			parent.velocity.y = -parent.jump_velocity

			var jump_particles: GPUParticles3D = parent.JumpParticlesScene.instantiate()
			add_sibling(jump_particles)
			jump_particles.global_transform = global_transform

			do_squash_and_stretch(1.2, 0.1)

	var gravity: float = parent.jump_gravity if parent.velocity.y > 0.0 else parent.fall_gravity
	parent.velocity.y -= gravity * delta

	var in_the_air: bool = not parent.is_on_floor()
	...
	if not parent.is_on_floor() and was_on_floor and not Input.is_action_just_pressed(&"jump"):
		parent.transition_to(parent.AnimationState.FALL)
		coyote_timer.start()

	if parent.is_on_floor() and in_the_air:
...
func _on_hit_floor(y_vel: float) -> void:
	...
	var land_particles: GPUParticles3D = parent.LandParticlesScene.instantiate()
	add_sibling(land_particles)
	land_particles.global_transform = global_transform
	do_squash_and_stretch(0.7, 0.08)

func do_squash_and_stretch(value: float, timing: float = 0.1) -> void:
