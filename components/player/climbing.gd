...
func state_physics_process(delta: float) -> void:
	if is_instance_valid(climbing_object):
		...
		parent.global_position.z = move_toward(parent.global_position.z, climbing_object.global_position.z, delta * attachSpeed)
	parent.velocity = Vector3(0.0, climbSpeed if Input.is_action_pressed("up") else -climbSpeed if Input.is_action_pressed("down") else 0.0, 0.0)
	if Input.is_action_just_pressed("jump"):
		parent.state_chart.send_event("to_ground")
	
	var wasInAir: bool = not parent.is_on_floor()
	parent.move_and_slide()
	if wasInAir && parent.is_on_floor():
		parent.state_chart.send_event("to_ground")
