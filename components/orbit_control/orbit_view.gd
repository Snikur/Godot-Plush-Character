...
func _ready() -> void:
	...
	Global.mouse_sensitivity_changed.connect(func(new_sens: float) -> void:
		mouse_sensitivity = new_sens
	)
	joystick_sensitivity = Global.get_joystick_sensitivity()
	Global.joystick_sensitivity_changed.connect(func(new_sens: float) -> void:
...
func _process(delta: float) -> void:
	...
	var zoom_input: int = int(Input.is_action_just_released(&"zoom_camera_out")) - int(Input.is_action_just_released(&"zoom_camera_in"))
